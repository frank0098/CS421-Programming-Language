...
(define value-of-operand
  (lambda (exp env)
    (cases expression exp
           (variable (var) (apply-env env var)) ; no deref!
           (num0 (num) (newref (value exp env)))
           (proc-exp (var body) (newref (value exp env)))
           (else
      ...
      (proc-exp (var body)
       ...
       (call-exp (rator rands)
                     (let ((proc (expval->proc (value rator env)))
                           (args (map (lambda(x) (value-of-operand x env))
                                      rands)))
...
#(i "letrec f = proc (x) if =(x , 0) then 1 else *(x, (g -(x, 1)))
g = proc (x) if =(x , 0) then 2 
else *(x, (f -(x, 1))) in(f 3)")
#(i "letrec
        even = proc(x) if =(x,0) then 1 else (odd -(x,1))
        odd=proc(x) if =(x,0) then 0 else (even -(x,1))
      in (odd 12)")
